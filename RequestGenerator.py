...
def make_raw_request(headers, post_params_bytes):
	...
	if environ["REQUEST_METHOD"] == "POST":
		raw_request += str(post_params_bytes,
						   encoding='utf-8')  # Adds the parameters provided to POST requests.
	return raw_request
...
def get_headers():
	...
	for field in environ:
		if field.startswith(
				"HTTP_"):  # If an environment variable starts with HTTP_ it is a header.
...
def get_post_params():
	...
	if environ[
		...
		post_params_dict = urllib.parse.parse_qs(
			str(post_params_bytes, encoding='utf-8'))
