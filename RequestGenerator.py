...
def generate_request():
    ...
    headers = get_headers()
    post_params_str, post_params_dict = get_post_params()
    raw_request = make_raw_request(headers, post_params_str)
    return Request(environ["REMOTE_ADDR"], environ["REMOTE_PORT"], raw_request, environ["REQUEST_METHOD"],
                   environ["REQUEST_URI"], headers, post_params_str, post_params_dict)


def make_raw_request(headers, post_params_str):
    ...
    raw_request += "\r\n" + post_params_str  # Adds the parameters provided to POST requests.
    return raw_request
...
def get_post_params():
    ...
    if environ["REQUEST_METHOD"] == "POST":  # If the request is a POST request...
        post_params_str = sys.stdin.read()
        post_params_dict = urllib.parse.parse_qs(post_params_str)
        return post_params_str, post_params_dict
    else:
        return "", {}
